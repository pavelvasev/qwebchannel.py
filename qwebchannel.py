...
class QObject(object):
    ...
    def __init__(self, name, data, webChannel):
        ...
        def unwrapQObject(response):
            ...
            if objectId in webChannel.objects.keys():
                return webChannel.objects[objectId]

            if response["data"] is None:
                print("Cannot unwrap unknown QObject " +
                      objectId + " without data.")
                return
        ...
        def unwrapProperties():
            for propertyIdx in self.__propertyCache__.keys():
                self.__propertyCache__[propertyIdx] = self.unwrapQObject(
                    self.__propertyCache__[propertyIdx])
        ...
        def addSignal(signalData, isPropertyNotifySignal):
            ...
            def __disconnect_func(callback):
                ...
                try:
                    idx = _object.__objectSignals__[
                        signalIndex].index(callback)
                except:
                    idx = -1

                if idx == -1:
                    print("Cannot find connection of signal " +
...
class QWebchannel(object):
    ...
    def __init__(self, transport, initCallback):
        ...
        def _init_callback(data):
            ...
            current_keys = list(self.channel.objects.keys())
            for objectName in current_keys:
        ...
        self.channel.exec(
            {"type": QWebChannelMessageTypes["init"]}, _init_callback)
    ...
    def handlePropertyUpdate(self, message):
        ...
        for i in message["data"].keys():
            ...
            if _object:
                ...
                print("Unhandled property update: " +
                      data["object"] + "::" + data["signal"])
        self.channel.exec({"type": QWebChannelMessageTypes["idle"]})

    def debug(self, message):
        self.channel.send(
            {"type": QWebChannelMessageTypes["debug"], "data": message})
